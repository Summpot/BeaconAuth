...
class ServerLoginHandler @JvmOverloads constructor(
    ...
    /**
     * Kick off an async lookup for the Mojang-verified UUID of the current profile. When this
     * returns true, the caller must defer [finish] until [consumeLookupResult] returns non-null
     * (or the tick timeout falls back to the plain Mojang UUID).
     */
    ...
    /**
     * Called from [tick] while a discount lookup is pending. Returns the resolved result when
     * available, or executes the default (keep Mojang UUID) after a timeout.
     */
    ...
    fun tick() {
        ...
        if (premiumLookupPending) {
            ...
            // Keep the login alive while the lookup runs; the async lookup itself enforces a
            // hard timeout and always delivers a fallback (not bound) result.
    ...
    fun handleCookieResponse(key: Identifier, payload: ByteArray?): Boolean {
        ...
        when (type) {
            ...
            LoginQueryType.LOGIN_URL -> {
                // LOGIN_URL is not used on 26.2 because cookie requests are request-only.
