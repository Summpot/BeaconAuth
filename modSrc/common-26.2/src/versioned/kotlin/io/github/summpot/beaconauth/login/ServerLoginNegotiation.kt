 ...
 *
 * 26.2 uses cookie request/response where the key is a ResourceLocation.
 */
class ServerLoginNegotiation {
    ...
    private val transactions: MutableMap<Identifier, LoginQueryType> = HashMap()
    ...
    fun registerTransaction(key: Identifier, type: LoginQueryType) {
        transactions[key] = type
    }

    fun consume(key: Identifier): LoginQueryType? = transactions.remove(key)
