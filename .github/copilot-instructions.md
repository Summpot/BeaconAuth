...
* **Token Verification:**
    ...
    * **`POST /api/v1/passkey/auth/start`**: Receives optional `{ username }`. Starts passkey authentication. Returns `{ "request_options": RequestChallengeResponse }`. Stores auth state in moka cache (5-min TTL, keyed by **base64url-encoded** (no padding) challenge from `rcr.public_key.challenge`). **CRITICAL**: Must use `BASE64URL` encoder, not `BASE64`, to match WebAuthn client format.
    * **`POST /api/v1/passkey/auth/finish`**: Receives `{ credential }`. **CRITICAL**: The challenge must be extracted from `client_data_json` by first decoding it as UTF-8, parsing as JSON, then accessing the `challenge` field (which is already base64url-encoded without padding). This challenge is used directly to retrieve the stored `PasskeyAuthentication` state from cache. Completes passkey auth, updates credential counter and last_used_at. Creates session tokens and returns them as cookies. Returns `{ "success": true, "username": str }`.
