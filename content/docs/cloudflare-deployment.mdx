...
If you want the UI and API to share a single origin, add a Pages Functions worker at
`src/_worker.ts` to proxy `/api/*`, `/v1/*`, and `/.well-known/*` to the Worker service binding
(`BACKEND`). The entry is already wired in `rslib.config.ts`.
