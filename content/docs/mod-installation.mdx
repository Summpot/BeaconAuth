...
[behavior]
# Dual-path online-mode (recommended true):
# - Leave HELLO to Mojang first (do not consume it).
# - Mojang success → vanilla / verified clients allow-through (keep Mojang UUID).
...
- Online-mode dual-path: premium clients (with or without the mod) keep Mojang UUIDs; offline clients with the mod fall back to the BeaconAuth web flow after Mojang rejects the session.
