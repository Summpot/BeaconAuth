...
| `--microsoft-client-id` | `MICROSOFT_CLIENT_ID` | Microsoft Entra ID client ID. Also reused for the Minecraft (Xbox) OAuth flow. |
| `--microsoft-client-secret` | `MICROSOFT_CLIENT_SECRET` | Microsoft Entra ID client secret. |
| `--microsoft-tenant` | `MICROSOFT_TENANT` | Defaults to `common`. Used by the Entra `microsoft` provider only; the `minecraft` provider always uses the consumer tenant. |
| `--minecraft-identity-mode` | `MINECRAFT_IDENTITY_MODE` | Default per-user Minecraft identity preference: `mojang` (default) or `legacy`. |
| `--minecraft-lookup-secret` | `MINECRAFT_LOOKUP_SECRET` | Shared secret the Minecraft mod presents to `GET /api/v1/minecraft/lookup`. Empty disables the endpoint. |
