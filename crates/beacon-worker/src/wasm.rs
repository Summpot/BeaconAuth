...
pub async fn fetch(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    ...
    if method == Method::Post && path == "/v1/minecraft/link/verify" {
        return handlers::minecraft::handle_minecraft_link_verify(req, &env).await;
    }
    if method == Method::Post && path == "/v1/minecraft/link/confirm" {
        return handlers::minecraft::handle_minecraft_link_confirm(req, &env).await;
    }
    if method == Method::Post && path == "/v1/minecraft/identity-mode" {
        return handlers::minecraft::handle_minecraft_identity_mode(req, &env).await;
    }
