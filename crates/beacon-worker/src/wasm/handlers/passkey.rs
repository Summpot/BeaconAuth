...
use crate::wasm::{
    ...
    db::{
        db_connect, db_put_passkey_state, db_take_passkey_state, db_delete_passkey_by_id,
        db_insert_passkey, db_passkey_by_credential_id, db_insert_refresh_token,
        db_passkey_by_id, db_passkeys_all, db_passkeys_by_user_id, db_update_passkey_usage,
        db_user_by_id, passkey_auth_state_key, passkey_reg_state_key, PASSKEY_STATE_TTL_SECS,
    },
...
pub async fn handle_passkey_register_start(mut req: Request, env: &Env) -> Result<Response> {
    ...
    let existing_passkeys = db_passkeys_by_user_id(&db, &user_id).await?;
...
pub async fn handle_passkey_register_finish(mut req: Request, env: &Env) -> Result<Response> {
    ...
    let passkey_id = match db_insert_passkey(&db, &user_id, &credential_id_b64, &credential_data, &name).await {
        Ok(id) => id,
        Err(e) => {
            let msg = e.to_string();
...
pub async fn handle_passkey_auth_start(mut req: Request, env: &Env) -> Result<Response> {
    ...
    let (allow_credential_ids, has_any_passkeys) = if let Some(username) = username {
        let Some(user) = crate::wasm::db::db_user_by_username(&db, &username).await? else {
            return error_response(&req, 404, "user_not_found", "User not found");
        };
        let passkeys = db_passkeys_by_user_id(&db, &user.id).await?;
        ...
        let passkeys = db_passkeys_all(&db).await?;
...
pub async fn handle_passkey_auth_finish(mut req: Request, env: &Env) -> Result<Response> {
    ...
    let raw_id_bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
        ...
        .map_err(|_| Error::RustError("Invalid credential rawId".to_string()))?;
    let credential_id_b64 = BASE64.encode(&raw_id_bytes);
    let Some(passkey_row) = db_passkey_by_credential_id(&db, &credential_id_b64).await? else {
    ...
    let used_ts = now_ts();
    db_update_passkey_usage(&db, &passkey_row.id, &updated_data, used_ts).await?;

    let Some(user) = db_user_by_id(&db, &passkey_row.user_id).await? else {
        return error_response(&req, 404, "user_not_found", "User not found");
    };
...
pub async fn handle_passkey_list(req: &Request, env: &Env) -> Result<Response> {
    ...
    let passkeys = db_passkeys_by_user_id(&db, &user_id).await?;
...
pub async fn handle_passkey_delete_by_id(req: &Request, env: &Env, id: String) -> Result<Response> {
    ...
    let Some(passkey) = db_passkey_by_id(&db, &id).await? else {
        return error_response(req, 404, "passkey_not_found", "Passkey not found");
    };
    ...
    db_delete_passkey_by_id(&db, &id).await?;
